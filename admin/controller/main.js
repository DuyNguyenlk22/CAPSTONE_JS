...
let addPhone = () => {
    ...
    // start validate
    //tenSV
    let isValidID = kiemTraRong("tbID", data.id) && kiemTraDoDai(4, 6, "tbID", data.id) && kiemTraDuLieuLaSo("tbID", data.id);
    let isValidPhoneName = kiemTraRong("tbName", data.name);
    let isValidPrice = kiemTraRong("tbPrice", data.price) && kiemTraDuLieuLaSo("tbPrice", data.price);
    ...
    //
    let isValid = isValidID & isValidPhoneName & isValidPrice & isValidScreen & isValidBackCam & isValidFontCam & isValidIMG & isValidDesc & isValidSelect;
    if (!isValid) return;
